...
cat("The balanced accuracy is", svm_rad_pca_balanced_accuracy, "\n") # 0.9411
```

```{r svm_radial_pca_misclass}
svm_misclassification_rate <- mean(svm_rad_pca_pred != test_data$label)
cat("SVM Misclassification Rate:", svm_misclassification_rate, "\n") # 0.0447
```

```{r svm_radial_pca_accuracy}
svm_radial_pca_accuracy <- mean(svm_rad_pca_pred == test_data$label)
cat("SVM Accuracy:", svm_radial_pca_accuracy, "\n") # 0.9553
```

```{r svm_radial_pca_roc}
roc_svm <- roc(test_data$label, as.numeric(svm_rad_pca_pred))
auc(roc_svm)
plot(roc_svm, main = "SVM ROC Curve") # Make the limits of the axes 0-1
```

```{r svm_radial_pca_f1_score}
svm_rad_pca_f1_score <- F1_Score(y_pred = svm_rad_pca_pred, 
                         y_true = test_data$label,
                         positive = 'TREG')
cat("F1 Score:", svm_rad_pca_f1_score, "\n") # 0.9645
```
...
log_pca_metrics_table <- data.frame(
  Metric = c("Accuracy", "Balanced Accuracy", "F1 Score", "AUC (ROC)", "Misclassification Rate"),
  Value = c(svm_radial_pca_accuracy, log_pca_balanced_accuracy, log_pca_f1_score, log_pca_auc, log_pca_misclas)
)
print(log_pca_metrics_table)
```
