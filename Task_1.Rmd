...
mean_table <- task_one_df %>% 
  ...
  mutate(diff = abs(mean_table$`CD4+T` - mean_table$TREG)) %>% 
...
# ## Pruning, does it improve anything? 
prune.rna_features <- prune.tree(tree.rna_features, best = 20)
# plot(tree.rna_features)
# text(tree.rna_features, pretty = 0)
