  ...
  for (fold in fold_indices) {
    ...
    for (prin_comps in no_prin_comps){
      ...
      logreg_cv_model <- glmnet(as.matrix(fold_pca_model$x[, 1:prin_comps]),
                                as.factor(task_one_df[fold, 'label']),
                                family = "binomial",
                                alpha = alpha)
...
logreg_cv_scores <- sapply(c(0, 1), 
                           function(alpha){fun_logreg_cv(no_prin_comps = c(10:30), alpha = alpha, lambda = lambda_tuning)})
logreg_cv_time <- Sys.time() - logreg_cv_time 
```
