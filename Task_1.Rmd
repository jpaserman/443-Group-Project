...
```{r svm_Misclass}
svm_misclas <- mean(yhat.svm != test_data$label)
...
lda2_test_data <- lda2_test_data[, !(names(lda2_test_data) %in% c("label_as_factor", "label_as_numeric"))]
...
# Prepare the input data (removing the target column from the predictor variables)
...
logreg_pred_regularized <- ifelse(logreg_probs_regularized > 0.5, 'TREG', 'CD4+T')

```

```{r}
log_confusion_regularized <- table(prediction = logreg_pred_regularized, truth = test_data$label)
log_confusion_regularized
```
