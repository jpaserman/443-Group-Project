...
```{r rf_misclass}
rf_misclassification_rate <- mean(yhat.rf != test_data$label)
...
summary_without_PCA <- list(
    ...
    auc = c(lda_auc, log_auc, qda_auc, knn_auc, gbm_auc, rf_auc, svm_auc),
    misclas = c(lda_misclas, log_misclas, qda_misclas, knn_mislas, gbm_misclas, rf_misclas, svm_misclas),
...
```{r LDA_with_PCA}
lda_pca_model <- lda(label ~ ., data = train_data_pca)
...
```{r lda_pca_f1_score}
lda_pca_f1_score <- F1_Score(y_true = test_data_pca$label, y_pred = lda_pca_pred$class)
cat("F1 Score of LDA with PCA:", lda_pca_f1_score, "\n")
```

```{r lda_pca_roc_auc}
roc_curve_lda_pca <- roc(test_data_pca$label, lda_pca_pred$posterior[,2], levels = c(1, 0), direction = ">")
...
```{r log_pca_model}
log_pca_model <- glm(label ~ ., data=train_data_pca, family=binomial)
...
```{r log_pca_roc_auc}
roc_curve_log_pca <- roc(test_data_pca$label, log_pca_pred, levels = c(1, 0), direction = ">")
...
```{r}
log_confusion_ridge <- table(prediction = logreg_pred_ridge, truth = test_data$label)
log_confusion_ridge
```

```{r log_pca_accuracy}
log_ridge_accuracy <- mean(log_pca_pred == pca_test_data$label)
cat("Accuracy of Logistic Regression with Ridge:", log_ridge_accuracy, "\n")
```

```{r log_pca_balanced_accuracy}
log_ridge_balanced_accuracy <- fun_calc_balanced_accuracy(log_confusion_ridge,
                                                        pos_label = "TREG",
                                                        neg_label = "CD4+T")
cat("Balanced Accuracy of Logistic Regression with PCA:", log_pca_balanced_accuracy, "\n")
```
...
```{r log_pca_misclas}
log_pca_misclas <- mean(log_pca_pred != pca_test_data$label)
cat("Misclassification rate of Logistic Regression with PCA:", log_pca_misclas, "\n")
```
