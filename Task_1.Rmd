...
qda_pca_fit <- qda(label ~ . , data = train_data_pca)
qda_runtime <- Sys.time() - qda_start_time
```

```{r qda_pca_confusion}
qda_pca_pred <- predict(qda_pca_fit, test_data_pca)$class

table(predicted = qda_pca_pred, observed = test_data$label)
```

```{r qda_pca_misclass}
qda_pca_misclass_rate = mean(qda_pred != test_data$label)
