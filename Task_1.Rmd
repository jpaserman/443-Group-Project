...
task_one_df_pca <- task_one_df_scaled %>% 
  ...
  prcomp(center = FALSE, scale. = FALSE, rank. = 10)

pca_end <- Sys.time()

pca_runtime <- pca_end - pca_start # Time difference of 3.813981 mins

# Split into training and test data
train_data_pca <- as.data.frame(task_one_df_pca$x[train_indices,])

test_data_pca <- as.data.frame(task_one_df_pca$x[-train_indices,])
...
```{r qda_pca_misclass}
qda_pca_misclass_rate = mean(qda_pca_pred != test_data$label)

cat("Misclassification rate:", qda_pca_misclass_rate*100, "%")
```
...
#### QDA? - confirm theory to identify techniques of improvement (Chi)
