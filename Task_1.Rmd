...
lda_best_no_pc <- 26 #as.integer(lda_improved_models[1, 'no_pc'])
...
boost_cv_scores <- fun_boost_cv(no_prin_comps = c(25))
...
```{r}
write.csv(boost_cv_scores, file = "predictions_1.3/boost_cv_scores_chosen_pc.csv")
```
...
```{r}
boost_avg_scores <- boost_cv_scores %>%
...
boost_tuned_pred <- predict(boost_tuned_model, 
                            newdata = data.frame(test_boost_tuned))$class
```
...
mypredict <- function(){
  ...
  model <- glmnet(as.factor(train_df$label) ~ .,
  ...
  threshold <- 0.4
  predictions <- ifelse(probabilities > threshold, "TREG", "CD4+T")
