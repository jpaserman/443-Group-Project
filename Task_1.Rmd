...
knn_train_X <- task_one_df_pca[train_indices,1:45] 

knn_train_Y <- task_one_df$label[train_indices]

knn_test_X <- task_one_df_pca[-train_indices,1:45] 

knn_start_time <- Sys.time()
knn_pca_pred <- sapply(c(20:35),
                       function(kk){
                       knn(train = knn_train_X, 
                           test = knn_test_X, 
                           cl = knn_train_Y, 
                           #k = 5
                           k = kk
...
```{r kNN_ROC}
f1 <- c()
for (i in 1:length(c(20:35))){
  
  score <- F1_Score(y_true = test_data$label,
                    y_pred = knn_pca_pred[, i],
                    positive = 'TREG')
...
knn_pca_misclass = mean(knn_pca_pred != test_data$label)

knn_pca_accuracy <- mean(knn_pca_pred == test_data$label)

knn_pca_roc_curve <- roc(response = test_data$label_as_numeric, 
                 predictor = knn_pred_numeric,
                 levels = c(1, 0),
                 direction = ">"
                 )

knn_pca_auc = knn_pca_roc_curve$auc

knn_pca_balanced_accuracy <- fun_calc_balanced_accuracy(knn_pca_confusion,
                                                        pos_label = 'TREG',
                                                        neg_label = 'CD4+T')

knn_pca_f1_score <- F1_Score(y_true = test_data$label, 
                             y_pred = knn_pca_pred,
                             positive = "TREG")
...
knn_pca_metrics_table <- data.frame(
  Metric = c("Accuracy", "Balanced Accuracy", "F1 Score", "AUC (ROC)", "Misclassification Rate"),
  kNN = c(knn_pca_accuracy, knn_pca_balanced_accuracy, knn_pca_f1_score, knn_pca_auc, knn_pca_misclass)
)

print(knn_pca_metrics_table)

print(left_join(knn_pca_metrics_table, qda_pca_metrics_table, by = "Metric"))
```
...
##### Cross-validation and picking n PCs
...
for (prin_comps in no_prin_comps) {
  ...
  X_pca <- task_one_df_pca[train_indices, 1:prin_comps]  # Use the first n_components
  ...
  model_result <- lda_model$result
  
  return_df <- data.frame(no_pc = c(rep(prin_comps, length(model_result$F1))),
...
```{r}
ggplot(lda_cv_scores, aes(x = no_pc, y = f1)) +
  geom_line() +
  geom_point()
```
...
X_pca <- task_one_df_pca[train_indices, 1:lda_best_no_pc]
...
lda_improved_model <- train(label ~ ., 
                   data = data_pca, 
                   method = "lda", 
                   trControl = train_control,
                   metric = "F1")

lda_improved_pred <- predict(lda_improved_model, newdata = test_lda_improved)
```

```{r lda_improved_confusion}
lda_improved_confusion <- table(predicted = lda_improved_pred,
                           observed = test_data$label)

print(lda_improved_confusion)
```

```{r lda_improved_diagnostics}
lda_improved_probs <- predict(lda_improved_model, newdata = test_lda_improved, type = "prob")

lda_improved_accuracy <- mean(lda_improved_pred == test_data_pca$label)

lda_improved_balanced_accuracy <- fun_calc_balanced_accuracy(lda_improved_confusion,
                                                        pos_label = 'TREG',
                                                        neg_label = 'CD4+T')

lda_improved_f1_score <- F1_Score(y_true = test_data_pca$label, 
                             y_pred = lda_improved_pred,
                             positive = "TREG")

lda_improved_roc_curve <- roc(
  response = test_data_pca$label,
  predictor = lda_improved_probs[,'TREG'], 
  levels = c('CD4+T','TREG'),
  direction = "<"
  )

lda_improved_auc <- auc(lda_improved_roc_curve)

lda_improved_misclas <- mean(lda_improved_pred != test_data_pca$label)

plot(lda_improved_roc_curve, col = "red", main = "ROC Curve for QDA")
```

```{r lda_improved_evaluation}
# Create a table of the evaluation metrics
lda_improved_metrics_table <- data.frame(
  Metric = c("Accuracy", "Balanced Accuracy", "F1 Score", "AUC (ROC)", "Misclassification Rate"),
  LDA_Improved = c(lda_improved_accuracy, lda_improved_balanced_accuracy, lda_improved_f1_score, lda_improved_auc, lda_improved_misclas)
)

print(lda_improved_metrics_table)
```
##### Boosting an LDA 
...
cm_rf_ab <- confusionMatrix(yhat.rf_ab, test_data$label)
cat("F1 Score on Test Set (AdaBoost with Random Forest): ", cm_rf_ab$byClass["F1"], "\n")
```
...
```{r}
rf_cv_f1_score <- F1_Score(y_true = test_data$label, y_pred = rf_ab_pred)
