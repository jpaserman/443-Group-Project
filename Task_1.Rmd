...
svm_pca_balanced_accuracy <- fun_calc_balanced_accuracy(svm_confusion_matrix,
                                                    pos_label = "TREG",
                                                    neg_label = "CD4+T")
...
```{r svm_ROC_AUC}
svm_roc <- roc(test_data$label, as.numeric(svm_pca_pred))
svm_auc <- auc(svm_roc)
cat("SVM AUC:", svm_auc, "\n")
plot(svm_roc, main = "SVM ROC Curve") # Make the limits of the axes 0-1
```
...
```{r rf_f1_score}
svm_f1_score <- F1_Score(y_pred = svm_pca_pred, y_true = test_data$label)
cat("F1 Score:", svm_f1_score, "\n") # 0.9645
```
