...
Number of trees (n_estimators): Increasing the number of trees generally improves accuracy, but can slow down the model. 
Maximum tree depth (max_depth): Controls how deep each tree can grow, preventing overfitting with a smaller depth. 
Minimum samples per leaf (min_samples_leaf): Sets the minimum number of samples required to split a node, preventing overfitting. 
Number of features considered at each split (max_features): Controls the randomness in the feature selection process. 
```{r}
