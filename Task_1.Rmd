...
lda_fit <- lda(label_as_numeric ~ . -label -label_as_factor, data=train_data)
...
```{r Confusion matrix}
table(lda_pred, truth = test_data$label)
```

```{r Misclassification error rate}
mean(lda_pred != test_data$label_as_numeric)
```
...
table(prediction = glm_pred, truth = test_data$label_as_factor)
```

```{r log regression Missclassification error rate}
mean(glm_pred != test_data$label_as_factor)
```
...
The accuracy for the gradient descent decision tree model (where the shrinkage term was equal to 0.001) was 0.904. The Area under the curve was equal to 0.972 and the Test mean squared error was equal to 0.109. Misclassification rate was equal to 0.096. And F1 score was equal to 0.928
...
gbm_test_data_scaled <- scale(test_data_cleaned_gbm[, -which(names(test_data_cleaned_gbm) %in% c("label", "label_as_factor"))])
