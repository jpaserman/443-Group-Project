...
logistic_ridge_train_data <- task_one_df_pca[train_indices,]

logistic_ridge_train_data$label <- train_data$label %>%
  make.names() %>%
  as.factor()
...
set.seed(seed)
logistic_ridge_model <- train(x = x_train_log_regularized, 
                              y = as.factor(make.names(train_data$label)), 
                              method = "glmnet",
                              trControl = train_control,
                              tuneGrid = expand.grid(
                                alpha = c(0, 1),
                                # alpha=1 for Lasso, alpha=0 for Ridge,
                                # or between 0 and 1 for Elastic Net
                                lambda = c(0.0001, 0.0005,
                                           0.001, 0.005,
                                           0.01, 0.02)
                                )
                             )
...
logreg_run_time <- Sys.time() - logreg_start_time # Time difference of 1.035202 mins

# Make predictions on the test data
logistic_ridge_test_data <- task_one_df[-train_indices,]

logistic_ridge_test_data$label <- test_data$label %>%
  make.names() %>%
  as.factor()

logreg_probs_regularized <- predict(logistic_ridge_model, 
                                    newx = as.matrix(logistic_ridge_test_data), 
                                    type = "prob")

logreg_pred_ridge <- ifelse(logreg_probs_regularized[,2] > 0.5, 'TREG', 'CD4+T')
```
