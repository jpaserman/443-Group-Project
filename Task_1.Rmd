...
```{r qdaTrain}
covariates = mean_table$genes[1:300]
...
```{r qdaMisclass}
mean(qda_pred != test_data$label)
```

```{r qdaROC}
roc_curve_qda <- roc(formula = qda_fit$)
plot(roc_curve_lr, col = "blue", main = "ROC Curve for Logistic Regression")
auc(roc_curve_qda)
```
...
```
The accuracy for the gradient descent decision tree model (where the shrinkage term was equal to 0.001) was 0.904. The Area under the curve was equal to 0.972 and the Test mean squared erroe was equal to 0.109. 
