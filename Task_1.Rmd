...
```{r}
lda_pca_start <- Sys.time()
lda_pca_model <- prcomp(train_data_log, scale = TRUE)
lda_pca_end <- Sys.time()
lda_pca_total_time <- lda_pca_end - lda_pca_start
lda_pca_total_time

names(lda_pca_model)
lda_pca_model$center
# lda_pca_model$scale
# lda_pca_model$rotation
# dim(lda_pca_model$x)

log_pca_var <- log_pca_model$sdev^2
log_pca_pve <- log_pca_var/sum(log_pca_var)
log_pca_pve
cumulative_pve <- cumsum(log_pca_pve)
num_components_90 <- which(cumulative_pve >= 0.90)[1]
num_components_90
selected_components <- 1:num_components_90
