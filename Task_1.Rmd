...
lda_fit <- lda(label_as_factor ~ . -label -label_as_numeric, data=train_data_lda)
...
```{r lda_accuracy}
lda_accuracy <- mean(lda_pred == test_data$label_as_factor)
...
```{r lda_f1_score}
lda_f1_score <- F1_Score(y_true= test_data$label_as_factor, y_pred = lda_pred)
...
# Standardize the data (important for PCA)
rf_train_data_scaled <- scale(train_data[, -which(names(train_data) %in% c("label", "label_as_factor"))])
rf_test_data_scaled <- scale(test_data[, -which(names(test_data) %in% c("label", "label_as_factor"))])
...
rf_train_data_pca$label_as_factor <- train_data$label_as_factor  # Add the target variable

# Transform the test data using the PCA model
rf_test_data_pca <- as.data.frame(predict(rf_pca_model, newdata = rf_test_data_scaled)[, 1:10])
rf_test_data_pca$label_as_factor <- test_data_cleaned_for_rf$label_as_factor  # Add the target variable
```
...
rf_pca_model <- randomForest(label_as_factor ~ ., data = rf_train_data_pca, mtry = 3, importance = TRUE, n.tree = 5000)
...
```{r rf_pca_Confusion}
# Confusion Matrix
rf_confusion_matrix_pca <- table(Predicted = yhat_rf_pca, Actual = rf_test_data_pca$label_as_factor)
...
```{r rf_pca_Misclass}
# Misclassification Rate
rf_misclassification_rate_pca <- mean(yhat_rf_pca != rf_test_data_pca$label_as_factor)
cat("Random Forest Misclassification Rate with PCA:", rf_misclassification_rate_pca, "\n")
```
```{r rf_pca_accuracy}
# Accuracy
accuracy_rf_pca <- mean(yhat_rf_pca == rf_test_data_pca$label_as_factor)
cat("Random Forest Accuracy with PCA:", accuracy_rf_pca, "\n")
```

```{r rf_pca_ROC}
# ROC and AUC
roc_rf_pca <- roc(rf_test_data_pca$label_as_factor, as.numeric(yhat_rf_pca))
...
```{r rf_pca_F1}
# F1 Score
rf_f1_score_pca <- F1_Score(y_pred = yhat_rf_pca, y_true = rf_test_data_pca$label_as_factor)
