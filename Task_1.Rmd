...
for (prin_comps in no_prin_comps) {
  ...
  test_lda_tuning <- task_one_df_pca[-train_indices, 1:prin_comps]
  
  set.seed(seed)
    
  # Combine PCA-transformed data with the target
  data_pca <- data.frame(X_pca, label = make.names(train_data_pca$label))
  ...
  lda_cv_model <- train(as.factor(label) ~ ., 
                     data = data_pca, 
                     method = "lda", 
                     trControl = train_control,
                     metric = "ROC")
  
  lda_cv_probs <- predict(lda_cv_model, test_lda_tuning, type = "prob")
  
  for (threshold in thresholds){
    lda_cv_preds <- as.factor(ifelse(lda_cv_probs[, "TREG"] > threshold,
                                     1,
                                     0))
    
    f1 <- F1_Score(y_true = test_data$label_as_numeric,
                   y_pred = lda_cv_preds,
                   positive = 1)
...
lda_improved_models %>% filter(no_pc == 15)
```
