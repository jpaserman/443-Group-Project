...
lda_pca_metrics_table <- data.frame(
  Metric = c("Accuracy", "Balanced Accuracy", "F1 Score", "AUC (ROC)", "Misclassification Rate"),
  lda = c(lda_pca_accuracy, lda_pca_balanced_accuracy, lda_pca_f1_score, lda_pca_auc, lda_pca_misclas)
)
...
log_pca_metrics_table <- data.frame(
  Metric = c("Accuracy", "Balanced Accuracy", "F1 Score", "AUC (ROC)", "Misclassification Rate"),
  log = c(log_pca_accuracy, log_pca_balanced_accuracy, log_pca_f1_score, log_pca_auc, log_pca_misclas)
)
...
gbm_pca_pred <- ifelse(gbm_pred_probs_pca > 0.5, "TREG", "CD4+T")
...
gbm_pca_metrics_table <- data.frame(
  Metric = c("Accuracy", "Balanced Accuracy", "F1 Score", "AUC (ROC)", "Misclassification Rate"),
  gbm = c(gbm_pca_accuracy, gbm_pca_balanced_accuracy, gbm_pca_f1_score, gbm_pca_auc, gbm_pca_misclas)
)
...
rf_pca_metrics_table <- data.frame(
  Metric = c("Accuracy", "Balanced Accuracy", "F1 Score", "AUC (ROC)", "Misclassification Rate"),
  rf = c(rf_pca_accuracy, rf_pca_balanced_accuracy, rf_pca_f1_score, rf_pca_auc, rf_pca_misclas)
)
...
fun_lda_cvc <- function(no_prin_comps = c(2:25),
                        thresholds = seq(0.1, 0.9, 0.1),
                        no_folds = 5,
                        seed = 443){
  ...
  fold_indices <- createFolds(task_one_df$label, k = no_folds, returnTrain = TRUE)
  
  X_df <- task_one_df[, !colnames(task_one_df) %in% c("label", "label_as_numeric")]
  ...
  for (fold in fold_indices) {
    ...
    fold_train_df <- data.frame(fold_pca_model$x, 
                               label = task_one_df$label[fold]) 
    
    fold_test_X <- predict(fold_pca_model, X_df[-fold,])
    
    fold_test_Y <- task_one_df$label[-fold]
...
lda_cv_time <- Sys.time()
lda_cv_scores <- fun_lda_cvc(no_prin_comps = c(10:30))
lda_cv_time <- Sys.time() - lda_cv_time
...
```{r logreg_ridge_cv_train}
logreg_ridge_cv_time <- Sys.time()

logreg_ridge_cv_scores <- data.frame(no_pc = c(),
                            threshold = c(),
                            lambda = c(),
                            #balanced_accuracy = c(),
                            f1 = c())
...
for (prin_comps in no_prin_comps) {
  ...
  logreg_ridge_train_X <- task_one_df_pca[train_indices, 1:prin_comps] %>%
    
    as.matrix() # Use the first n_components
    
  for (fold in fold_indices){
    logreg_ridge_cv_model <- glmnet(logreg_ridge_train_X[fold,],
                              as.factor(train_data[fold, 'label']),
                              family = "binomial",
                              alpha = 0)
    
    for (lambda in lambda_tuning){
      logreg_ridge_cv_probs <- predict(logreg_ridge_cv_model,
                                 logreg_ridge_train_X[-fold,],
                                 type = "response",
                                 s = lambda)
      
      for (threshold in thresholds){
        logreg_ridge_cv_preds <- as.factor(ifelse(logreg_ridge_cv_probs > threshold,
                                         "TREG",
                                         "CD4+T"))
        
        # cm <- table(logreg_ridge_cv_preds, train_data[-fold, "label"])
        # 
        # balanced_accuracy <- fun_calc_balanced_accuracy(cm, 
        #                                                 pos_label = "TREG", 
        #                                                 neg_label = "CD4+T")
        ...
        # Store the scores
        logreg_ridge_cv_scores <- rbind(logreg_ridge_cv_scores, return_df)
      }
    }
  }
}
logreg_ridge_cv_time <- Sys.time() - logreg_ridge_cv_time # Time difference of 11.68833 secs
...
#### Logistic Regression with Lasso Regularisation and Cross Validation (Chi)

```{r logreg_lasso_cv_train}
logreg_lasso_cv_time <- Sys.time()

logreg_lasso_cv_scores <- data.frame(no_pc = c(),
                            threshold = c(),
                            lambda = c(),
                            #balanced_accuracy = c(),
                            f1 = c())
...
for (prin_comps in no_prin_comps) {
  ...
  logreg_lasso_train_X <- task_one_df_pca[train_indices, 1:prin_comps] %>%
    
    as.matrix() # Use the first n_components
    
  for (fold in fold_indices){
    logreg_lasso_cv_model <- glmnet(logreg_lasso_train_X[fold,],
                              as.factor(train_data[fold, 'label']),
                              family = "binomial",
                              alpha = 1)
    
    for (lambda in lambda_tuning){
      logreg_lasso_cv_probs <- predict(logreg_lasso_cv_model,
                                 logreg_lasso_train_X[-fold,],
                                 type = "response",
                                 s = lambda)
      
      for (threshold in thresholds){
        logreg_lasso_cv_preds <- as.factor(ifelse(logreg_lasso_cv_probs > threshold,
                                         "TREG",
                                         "CD4+T"))
        
        # cm <- table(logreg_lasso_cv_preds, train_data[-fold, "label"])
        # 
        # balanced_accuracy <- fun_calc_balanced_accuracy(cm, 
        #                                                 pos_label = "TREG", 
        #                                                 neg_label = "CD4+T")
        ...
        # Store the scores
        logreg_lasso_cv_scores <- rbind(logreg_lasso_cv_scores, return_df)
      }
    }
  }
}
logreg_lasso_cv_time <- Sys.time() - logreg_lasso_cv_time # Time difference of 32.19582 secs
...
```{r comparison to identify error}
logreg_start_time <- Sys.time()

# Prepare the input data
x_train_log_regularized <- task_one_df[train_indices,] %>% 
  dplyr::select(-label, -label_as_numeric)

y_train_log_regularized <- train_data$label_as_numeric

# Fit a regularized logistic regression model
logreg_fit_regularized_fixed <- glmnet(as.matrix(x_train_log_regularized),
                                 y_train_log_regularized, 
                                 family = "binomial",
                                 alpha = 0) 
# alpha=1 for Lasso, alpha=0 for Ridge, or between 0 and 1 for Elastic Net

logreg_run_time <- Sys.time() - logreg_start_time # Time difference of 16.91733 secs

# Make predictions on the test data
x_test_log_regularized <- task_one_df[-train_indices,] %>% 
  dplyr::select(-label, -label_as_numeric) %>% as.matrix()

logreg_probs_regularized <- predict(logreg_fit_regularized, 
                                    newx = x_test_log_regularized, 
                                    type = "response",
                                    s = 0.1)  # s = 0.1 is the regularization parameter
