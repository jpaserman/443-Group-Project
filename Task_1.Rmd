...
# Step 4: Train the LDA model using the PCA-transformed training data + test the model
lda_model <- lda(label ~ ., data = pca_data)
lda_predictions <- predict(lda_model, lda2_test_data)

# Step 5: Evaluate the model performance using confusion matrix
lda2_test_data$label <- as.factor(lda2_test_data$label)
lda_pca_confusion_mat <- confusionMatrix(lda_predictions$class, lda2_test_data$label)
print(confusion_mat_lda_pca)
```
