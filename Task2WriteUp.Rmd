...
Most of our exploratory data analysis was undertaken with the goal of preliminary feature reduction prior to model implementation. After confirming that there were no missing values within our data, we aim to remove features with extremely low to zero variance. The below histograms depict the number of compounds found to have bound to the target site and the variance associated with a given feature respectively. Notice how extremely right skewed both plots are, indicating a high density of features with low variance due to being largely associated with the negative class of compounds not binding. Features that contain little to no variance are not likely to be informative in explaining and as such, are not worthwhile to retain. By setting a variance threshold of 0.01, associated with a feature having less than or equal to 8 compounds binding out of 800, we are able to reduce the number of features in the dataset from 100,000 to only a little over 30,000. This will greatly speed up the subsequent feature reduction models and hopefully make our results more interpretable. However, even with decreasing our features to 30,000 there are some models that might struggle with the computational cost of training on the entire dataset. Therefore, we construct an additional subset of the dataset that solely contains the top 1,000 features that are highly correlated with the $label$. The only feature reduction models that utilize this extremely subset dataset are FSS and random forest, with the later solely using as a benchmark comparison as opposed to within the final model. You can assume otherwise if not specified, that the following models use the dataset that contains roughly 30,000 features. Lastly, an important aspect to note is that in terms of classes for the $label$ our data is very skewed with less than 10% (78/800) of the compounds being associated with the positive class. This structure of the data and its implications will be expanded upon later at the conclusion of the report.
...
While originally utilized solely for computational efficiency in order to run the "entire" 30,000 (explain how could only do the subset with the randomForest package that we previously used in seminar), we see that for the same parameter value (number of trees) the rnager package does much better in terms of selecting far fewer features, and better balanced accuracy

The balanced accuracy is tnot that much of a decrease for randomForest. Increasing our confidence in the decision to approach computational and memory errors by utilizing the subset dataset in FSS, likely still gave us comparable decent restulst

Note that while we use the subset dataset for random forest in a comparative measure, our final random forest model was trained on the "entireity" of the 30,000 features
...
#### XGBoost
Boosting is another technique that can be relevant for feature selection. Boosting combines weak learners (usually decision trees) to create a strong learner, and it does so by building models sequentially and attempting to correct the errors made n the previous model. While boosting, the algorithm records the importance of each feature based on how much it improved the model's performance. A boosting algorithm stores an importance score for each feature, and that way some features can be dropped. 
We attempted to use the XGBoost (Extreme Gradient Boosting) algorithm. XGBoost, is a boosting algorithm that is able to handle very large datasets and uses regularization techniques as well. Since XGBoost handles imbalanced data quite well, we decided to not weight the data. Our process is similar to the one used for Lasso and the Elastic net, where we loop over certain hyper parameters, most importantly $\lambda$, which represents the shrinkage parameters, specifically the rate which boosting learns. We then returned the model which returned the best balanced accuracy on the cross validation, and then used that model to make predictions on the test data. The number of features selected were extracted from the XGB importance matrix, which calculates importance based on gain, cover and frequency.^[Gain is the improvement in accuracy brought by a feature to the branches it is on. Cover represents the second derivative to the loss function with respect to a variable, a large value means the feature has a large importance on the loss function. Frequency represents the number of times a feature is used in all generated trees.].Features with 0, are excluded from the matrix. Below is a chart for the features selected for our best XGBoost Model. We found that the best learning rate, $\lambda$, was equal to 0.5!
...
scale and center... 
drop once again low variance features that appear in the training data split (at a 99/1 split threshold???)
show a handful of the different variable options and their computed accuracy, column for the selecfted model, and only 
for tha one do we report the balanced accuracy
