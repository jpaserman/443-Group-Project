...
# in a maximum of 8 out of 800 compounds), we would remove 69,879 features
...
table(task_two_df$label) # 722 compounds did not bind and 78 compounds did bind
...
```{r barcharts for top 3 correlated features}
top_1000_cor_minus_label <- top_1000_cor[-1,]
top_1000_cor_boxplot_data <- top_1000_cor_minus_label[order(-top_1000_cor_minus_label$abs_cor),]

top3_correlated <- top_1000_cor_boxplot_data[1:3,]

top_3_features <- top3_correlated$feature

new_df_2_top3_correlated_features <- task_two_df[, top_3_features]

# Melt to format data better
long_df <- melt(new_df_2_top3_correlated_features)

ggplot(long_df, aes(x = variable, fill = as.factor(value))) +
  geom_bar(position = "dodge") +
  geom_text(stat = "count", aes(label = ..count..), vjust = -0.5, position = position_dodge(width = 0.9)) +
  labs(fill = "Value of feature") +
  theme_minimal() +
  ggtitle("Distribution of Top 3 Features Correlated with the Label")
...
Our EDA incorporates initial feature selection by removing features that fall below a certain threshold of variance. This subsets our data from 100,000 explanatory features down to approximately 30,000 features. Additionally, we partition the data using a 80-20 split into a training and testing data set for upcoming feature selection models.
...
```
```{r extracting lambda values for lasso}
...
```
```{r fit on final model and return results}
...
plot(linear_lasso_roc, main = "Lasso ROC Curve")
```
...
# test the function
fss_test_data["value_bic"] <- predict_regsubsets(regfit_fwd, fss_test_data, id=best_model_bic)
fss_test_data["label_pred_bic"] <- ifelse(fss_test_data["value_bic"] < 0, -1, 1)

fss_test_data["value_adjr2"] <- predict_regsubsets(regfit_fwd, fss_test_data, id=best_model_adjr2)
fss_test_data["label_pred_adjr2"] <- ifelse(fss_test_data["value_adjr2"] < 0, -1, 1)

```

```{r fss_Confusion}
fss_confusion_matrix_bic <- table(predict = fss_test_data$label_pred_bic, truth = fss_test_data$label)
print(fss_confusion_matrix_bic)
fss_balanced_accuracy_bic <- fun_calc_balanced_accuracy(fss_confusion_matrix_bic,
                                                        pos_label = "1", 
                                                        neg_label = "-1")
cat("The balanced accuracy (BIC) is", fss_balanced_accuracy_bic, "\n") # 0.7804814


fss_confusion_matrix_adjr2 <- table(predict = fss_test_data$label_pred_adjr2, truth = fss_test_data$label)
print(fss_confusion_matrix_adjr2)
fss_balanced_accuracy_adjr2 <- fun_calc_balanced_accuracy(fss_confusion_matrix_adjr2,
                                                          pos_label = "1", 
                                                          neg_label = "-1")
cat("The balanced accuracy (adj r^2) is", fss_balanced_accuracy_adjr2, "\n") # 0.8257457
...
fss_auc <- auc(fss_roc)
cat("fss AUC:", fss_auc, "\n") # 0.7804814
plot(fss_roc, main = "fss ROC Curve")
...
```{r rf_misclass}
rf_misclassification_rate <- mean(yhat.rf != rf_test_data_top_subset$label)
cat("Random Forest (randomForest) Misclassification Rate:", rf_misclassification_rate, "\n") # 0.05 

rf_misclassification_rate_ranger <- mean(yhat.rf_ranger$predictions != rf_test_data$label)
cat("Random Forest (ranger) Misclassification Rate:", rf_misclassification_rate_ranger, "\n") # 0.06875 

```

```{r rf_accuracy}
rf_accuracy_ranger <- mean(yhat.rf == rf_test_data_top_subset$label)
cat("Random Forest (randomForest) Accuracy:", rf_accuracy_ranger, "\n") # 0.95

rf_accuracy_ranger <- mean(yhat.rf_ranger$predictions == rf_test_data$label)
cat("Random Forest (ranger) Accuracy:", rf_accuracy_ranger, "\n") # 0.93125 

```

```{r rf_roc_auc}
roc_rf <- roc(rf_test_data_top_subset$label, as.numeric(yhat.rf))
cat("Random Forest (randomForest) AUC:", auc(roc_rf), "\n")
plot(roc_rf, main = "Random Forest (randomForest) ROC Curve") # 0.7974882

roc_rf_ranger <- roc(rf_test_data$label, as.numeric(yhat.rf_ranger$prediction))
cat("Random Forest (ranger) AUC:", auc(roc_rf_ranger), "\n")
plot(roc_rf_ranger, main = "Random Forest (ranger) ROC Curve") # 0.8223443 

```

```{r rf_f1_score}
rf_f1_score <- F1_Score(y_pred = yhat.rf, y_true = rf_test_data_top_subset$label, positive = "1")
cat("F1 Score (randomForest):", rf_f1_score, "\n") # 0.6666667

rf_f1_score_ranger <- F1_Score(y_pred = yhat.rf_ranger$prediction, y_true = rf_test_data$label, positive = "1")
cat("F1 Score (ranger):", rf_f1_score_ranger, "\n") # 0.6206897
...
```
```{r print results for elastic net}
...
```
```{r elastic net test final model}
...
svm_rfe_vars <- predictors(svm_rfe) # A model with 35 features is selected to maximize the training accuracy (0.9422)
...
plot(roc_svm_rfe, main = "SVM with RFE ROC Curve") # 0.8574045
