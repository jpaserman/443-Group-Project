...
```{r remove_multicollinearity, cache=TRUE}
...
The best model according to maximizing RSS on the training data has 437 features
The best model according to minimizing BIC on the training data has 140 features

We will proceed with the model recommended by minimizing BIC (140 features).
...
```{r fss_f1_score}
fss_f1_score <- F1_Score(y_pred = fss_test_data$label_pred, y_true = fss_test_data$label)
...
rf.rna_features_ranger <- ranger::ranger(x = x, y = y, mtry = 5000, num.trees = 10000, importance = "impurity") # RF Model
...
cat("Number of features utilized in Random Forest model (randomForest):", rf_importance_ranger_vars, "\n") # This model has 23,063 "important" variables when utlizing the ranger library

```

We run into memory issues when trying to run randomForest::randomForest on the entirety of the task 2 training data (with 29788 features). As such, we proceed with only running the random forest on the subset of the top 1000 features that are most highly correlated with the label. After researching, found that conducting a random forest with ranger::ranger is much more computationally efficient (https://www.css.cornell.edu/faculty/dgr2/_static/files/R_html/CompareRandomForestPackages.html#4_Conclusion). When we use ranger:ranger we are able to build our randomForest on the entirity of the training data!
...
We notice that both the random forest built on only the subset of top 1000 features (randomForest), as well as the random forest built on the entire training data (ranger) have very similar results. This provides us with confidence in our approach of utilizing the subset of the training data for the FSS model. Moving forward for the random forest, we will utilize the ranger:ranger random forest built on the entire training data as it performs slightly better in terms of balanced accuracy.  

(or should we use the randomForest::randomForest because it is almost the same balanced accuracy yet only uses 755 instead of 23,063 features???)
...
plot(roc_rf, main = "Random Forest ROC Curve") # 0.794
```

```{r rf_f1_score}
rf_f1_score <- F1_Score(y_pred = yhat.rf_ranger$prediction, y_true = rf_test_data$label)
...
# Prepare the data
elastic_net_test_data_clean_matrix <- as.matrix(elastic_net_test_data[, -1])
elastic_net_train_data_matrix <- as.matrix(elastic_net_train_data[, -1])
...
# Perform cross-validation with a sequence of lambda values
start_task_two_elastic <- Sys.time()
elastic_cv_fit <- cv.glmnet(elastic_net_train_data_matrix, train_labels, alpha = 0.60, lambda = grid, nfolds = 5, family = "binomial", weights = class_weights)
end_task_two_elastic <- Sys.time()
total_elastic_time <- end_task_two_elastic - start_task_two_elastic
print(total_elastic_time)

# Extract the lambda values and their corresponding balanced accuracies
elastic_lambdas <- elastic_cv_fit$lambda
elastic_cv_results <- sapply(elastic_lambdas, function(lambda) {
  pred.cv <- predict(elastic_cv_fit, newx = elastic_net_train_data_matrix, s = lambda, family = "binomial", type ="response")
  elastic_pred.cv_binary <- ifelse(pred.cv > 0.5, 1, 0)
  confusion <- table(Predicted = elastic_pred.cv_binary, Actual = train_labels)
  balanced_acc <- calculate_balanced_accuracy(confusion)
  num_predictors <- sum(coef(elastic_cv_fit, s = lambda) != 0) - 1
  return(c(balanced_acc, num_predictors))
})
...
# Find the lambda with the highest balanced accuracy
elastic_best_lambda_index <- which.max(elastic_balanced_accuracies)
...
# Fit the final model using the best lambda
elastic_final_model <- glmnet(elastic_net_train_data_matrix, train_labels, alpha = 0.6, lambda = elastic_lambdas[elastic_best_lambda_index], family = "binomial", weights = class_weights)
elastic_pred.test <- predict(elastic_final_model, newx = elastic_net_test_data_clean_matrix, family ="binomial", type = "response")
...
cat("Best Lambda:", elastic_best_lambda, "\n")
cat("Highest Balanced Accuracy on CV:", elastic_best_balanced_accuracy, "\n")
cat("Number of Predictors for Best Lambda:", elastic_best_num_predictors, "\n")
cat("Balanced Accuracy on Test Data:", elastic_test_balanced_accuracy, "\n")
...
for (i in 1:nrow(grid_search)) {
  ...
  balanced_accuracies <- sapply(1:5, function(fold) {
    ...
    fold_predictions <- ifelse(xgb_cv$pred[fold_indices] > 0.5, 1, 0)
