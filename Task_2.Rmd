...
elastic_cv_fit <- cv.glmnet(elastic_net_train_data_matrix, train_labels, alpha = 0.60, lambda = grid, nfolds = 5, family = "binomial", weights = class_weights) # Performing a cross logistic elastic-net model (penalization of 0.6) with 5 folds
...
elastic_cv_results <- sapply(elastic_lambdas, function(lambda) {
  pred.cv <- predict(elastic_cv_fit, newx = elastic_net_train_data_matrix, s = lambda, family = "binomial", type ="response") # Predicting probabilities for each lambda from the cv model
  elastic_pred.cv_binary <- ifelse(pred.cv > 0.5, 1, 0) # Classifying the predictions to each class 1, if the probability was greater than 0.5, and 0 (-1) otherwise. 
  confusion <- table(Predicted = elastic_pred.cv_binary, Actual = train_labels)
  balanced_acc <- calculate_balanced_accuracy(confusion) # Calc balanced acc
  num_predictors <- sum(coef(elastic_cv_fit, s = lambda) != 0) - 1 #Calc num of coeffs 
  return(c(balanced_acc, num_predictors))
})
