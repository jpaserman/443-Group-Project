...
zero_var_cols <- apply(task_two_df, 2, var) <= 0.01
# which(zero_var_cols)
table(zero_var_cols)
...
```{r subset_data}
subset_index = as.integer(ncol(task_two_df)*0.15)
...
fit.lasso <-glmnet(x_data_sparse,train_data$label, alpha = 1, lambda = grid)
