...
```{r xgb_model_ save different params, and models}
# Initialize a list to store results
results <- list()

# Looping the xgboost model over different parameters
for (i in 1:nrow(grid_search)) {
  params <- list(
    eta = grid_search$eta[i],
    max_depth = grid_search$max_depth[i],
  ...
  # Store the results
  results[[i]] <- list(params = params, balanced_accuracy = xgb_mean_balanced_accuracy, nrounds = xgb_cv$best_iteration)
...
```
```{r worst_ba}
# Find the parameters with the worst balanced accuracy
worst_result <- results[[which.min(sapply(results, function(x) x$balanced_accuracy))]]
worst_params <- worst_result$params
worst_nrounds <- worst_result$nrounds
```
```{r xgb_worst_param_test}
# Train the model with the worst parameters
worst_model <- xgb.train(
  params = worst_params,
  data = xgb_train_matrix,
  nrounds = worst_nrounds
)

# Get feature importance for the worst model
worst_importance_matrix <- xgb.importance(feature_names = colnames(xgb_train_data[, -1]), model = worst_model)
worst_selected_features <- worst_importance_matrix$Feature

# Test predictions and new balanced accuracy for the worst model
worst_predictions <- predict(worst_model, newdata = xgb_test_matrix)
worst_predictions <- ifelse(worst_predictions > 0.5, 1, 0)

worst_confusion_matrix <- table(Predicted = worst_predictions, Actual = xgb_test_data$label)

# Calculate Balanced Accuracy for the worst model
worst_sensitivity <- worst_confusion_matrix[2, 2] / (worst_confusion_matrix[2, 2] + worst_confusion_matrix[1, 2])
worst_specificity <- worst_confusion_matrix[1, 1] / (worst_confusion_matrix[1, 1] + worst_confusion_matrix[2, 1])
worst_final_balanced_accuracy <- (worst_sensitivity + worst_specificity) / 2

# Output results for the worst model
cat("Final Balanced Accuracy for Worst XGboost Model:", worst_final_balanced_accuracy, "\n")
cat("Selected Features Count for Worst XGboost Model:", length(worst_selected_features), "\n")
cat("The worst parameters for our XGboost model were: eta:", worst_params$eta, "and max depth:", worst_params$max_depth, "\n")

# Plot Feature Importance for the Worst Model
xgb.plot.importance(worst_importance_matrix)
```
