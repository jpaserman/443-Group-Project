...
results <- sapply(grid, function(lambda) {
  fit.lasso <- glmnet(train_data_matrix, train_labels, alpha = 1, lambda = lambda, nfolds = 5)
  pred.test <- predict(fit.lasso, newx = test_data_clean_matrix)
  pred.test_binary <- ifelse(pred.test > 0, 1, -1)
  confusion <- table(Predicted = pred.test_binary, Actual = test_labels)
...
lambdas <- c(0.01, 0.1, 0.2, 0.02, 0.05)
depths <- c(2, 4, 6, 8)
